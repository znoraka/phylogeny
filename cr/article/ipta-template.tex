...
\section{Method}
In this section, we present a new approach to build an image phylogeny tree from a set of near-duplicate images. Unlike state of the art methods \cite{} that mainly focus on complex algorithm that try their best to approximate the tree from a dissimilarity matrix, we try to take a binary decision between two images : ``Can this image be an ancestor of that other one?''. It allows us to focus more on the images themselves and their characteristics and have a simple tree building algorithm. Figure \ref{fig:schema} shows the diagram of our method.
...
\begin{figure}[!ht]
  ...
  \scalebox{0.58}{
    \begin{tikzpicture}[auto,node distance=2.1cm]
      ...
      \node[draw=none,fill=none,minimum width=0.8in,text width=0.8in, align=center, left=0.4cm of F] (G) {$Q_f$ of each images};
      ...
      \node[state] (K) [left=0.4cm of J] {Tree building};
...
\begin{figure}[htbp]
  \begin{subfigure}{.5\textwidth}
    \centering
    \includegraphics[width=.5\linewidth]{algo_tree}
...
Algorithm \ref{algo_} presents our tree building algorithm.
...
\begin{algorithm}[]
  ...
  \KwRet treeRoot
\caption{Tree building algorithm.}
\label{algo_}
\end{algorithm}
