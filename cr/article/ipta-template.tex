...
Most state of the art methods \autocite{dias2010first, dias2012image, 7018370, 7472039} have a two steps method. First they compute a dissimilarity matrix. An asymetric matrix of distances between images and then compute a tree from this dissimilarity matrix. The dissimilarity matrix is built thanks to a dissimilarity function that yields small values for similar images. The tree is then constructed using minimum spanning trees algorithms.

...

Double JPEG compression state of the art.
...
\section{Method}
\label{sec:method}
In this section, we present a new approach to build an image phylogeny tree from a set of near-duplicate images. Unlike state of the art methods \cite{} that mainly focus on complex algorithm that try their best to approximate the tree from a dissimilarity matrix, we try to take a binary decision between two images : ``Can this image be an ancestor of that other one?''. It allows us to focus more on the images themselves and their characteristics and have a simple tree reconstruction algorithm. Figure \ref{fig:schema} shows the diagram of our method.
...
\begin{figure}[!ht]
  ...
  \scalebox{0.58}{
    \begin{tikzpicture}[auto,node distance=2.1cm]
      ...
      \node[draw=none,fill=none,minimum width=0.8in,text width=0.8in, align=center] (A) {Set de near-duplicates};
...
\begin{marqueur}
  A marker is a local or globale feature extracted from the image. This marker shows that a particular transformation was applied to the image. This marker is passed on the image children, and will be used to prove that an image is not an ancestor of another one.
...
\begin{proof}
  If $f(I_{m},I_{n})$ cannot find a marker that proves that $I_m$ is not an ancestor of $I_n$ then this marker does not exist, thus que $I_{m}$ is an ancestor of $I_{n}$, with $m < n$.
\end{proof}
...
\caption{Autocorrelation of the DC coefficient and the threshold in green}
...
The main advantage of checking against every table is that it outputs a $Q_f$, it is the $n$ the index of the minimal distance between $q_n(u,v)$ and $\widehat{q}(u,v),\ n\in \{1..100\}$. It however is computation heavy and lacks accuracy. 
...
if $I_m = C(I_n, Q_{f_{m}})$, then $I_n$ is a direct ansestor of $I_m$.
...
We use seven metrics to measure our results, three metrics measuring the accuracy of the estimation of the quality factor, the four other are given by \citeauthornsc{dias2010first} \autocite{dias2010first} and available table \ref{fig:metrics}.
...
Table \ref{results} shows the results we got using our methods on the three datasets.
...
\begin{figure}[htbp]
  ...
  \begin{tabular}{|l||c|c|c|c|c|}
    ...
    \textbf{\pbox{2.6cm}{Mean estimation error of $Q_f$}} & 0.52 & 0.64 & 0.83 \\ \hline
...
We can see that the root metric suffers the most from the image missing. As explained above, our tree reconstruction algorithm cannot differentiate an image with no parents (the root) and an image with no parents detected. The root of the tree is in this case almost randomly chosen. Figure \ref{fig:missing} shows how a tree with a missing node results in an incorrect tree estimation. We actually have correct subtrees, with the right parents, but we do not have the information on how to links those tree together.
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{.5\textwidth}
    ...
    \end{forest}
    \caption{Output of tree reconstruction algorithm}
    \label{fig:missing1}
  \end{subfigure}

  \caption{Exemple of a tree with a missing node and a reconstruction attempt.}
  \label{fig:missing}
\end{figure}
