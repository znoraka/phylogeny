...
std::vector<std::vector<int> > readDCT(jpeg_decompress_struct srcinfo, jvirt_barray_ptr * src_coef_arrays, int coeff) {
  JBLOCKARRAY rowPtrs[MAX_COMPONENTS];
  ...
  for (JDIMENSION compNum=0; compNum < srcinfo.num_components; compNum++) {
	...
	for (JDIMENSION i=0; i<DCTSIZE2; i++){
	  ...
	  // vec.push_back(rowPtrs[compNum][0][blockNum][zigzag[i]]);
	  /* vec[i].push_back(rowPtrs[compNum][0][blockNum][zigzag[i]]); */
	  vec[i].push_back(rowPtrs[compNum][0][blockNum][i]);
